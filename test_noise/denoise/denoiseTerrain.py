...
class DenoiseTerrain(DenoiseBase):
    ...
    def denoise(src,
        ...
        terrain_denoise_image[:, :, 0] = Minnaert(radiance_B, sun_angle, slope, Minnaert_constant_B)
        terrain_denoise_image[:, :, 0] = radiance2DN(terrain_denoise_image[:, :, 0], gain_B, offset_B)

        terrain_denoise_image[:, :, 1] = Minnaert(radiance_G, sun_angle, slope, Minnaert_constant_G)
        terrain_denoise_image[:, :, 1] = radiance2DN(terrain_denoise_image[:, :, 1], gain_G, offset_G)

        terrain_denoise_image[:, :, 2] = Minnaert(radiance_R, sun_angle, slope, Minnaert_constant_R)
        ...
        if channels == 4:
            ...
            terrain_denoise_image[:, :, 3] = Minnaert(radiance_NIR, sun_angle, slope, Minnaert_constant_NIR)
